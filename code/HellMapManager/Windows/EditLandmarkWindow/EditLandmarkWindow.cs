...
public class EditLandmarkWindowViewModel : ObservableObject
{
    ...
    public void CancelEdit()
    {
        if (Raw is not null)
        {
            Item = new LandmarkForm(Raw.Clone(), Checker);
            Editing = false;
            OnPropertyChanged(nameof(Item));
            OnPropertyChanged(nameof(Editable));
            OnPropertyChanged(nameof(ViewMode));
    ...
    {
        if (AppKernel.MapDatabase.Current!.Cache.Landmarks.ContainsKey(model.UniqueKey) && (Raw is null || model.UniqueKey.ToString() != Raw.UniqueKey().ToString()))
        {
            return "定位主键已存在";
        }
        return "";
