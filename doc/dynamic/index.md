...
这两者在HMM其实是同一个东西，只是数据结构略有不同，更适合直接调用/序列化接口调用

上下文包含的内容包含

* Tags 标签，字符串>数字对，匹配 出口中的出口条件，动态的开关出口。
* RoomConditions 房间条件，与Tags想反，匹配房间的标签，动态的开关房间。
...
地图选项在HMM中是每次计算地图的参数，更注重单次计算的偏向性。

地图选项有以下内容

* MaxExitCost 最大出口消耗。适合在避免计算房间膨胀等情况下进入迷宫的效果。
* MaxTotalCost 最大路线消耗，避免太远路线。可避免在区域规划时进入迷宫等复杂情况。
* DisableShortcut 有些路径不能飞行，可以在这里进行开关。
* CommandWhitelist 指令的白名单。有些移动方式/载具只能执行有限的命令，可以通过这个参数进行限制。